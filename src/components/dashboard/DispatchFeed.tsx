...
export default function DispatchFeed() {
  ...
  useEffect(() => {
    // Simulate a new telegram arriving every 20 seconds
    const interval = setInterval(() => {
      const newEvent: Event = {
        id: Date.now(),
        icon: '✉️',
        date: new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }).toUpperCase(),
        text: NEW_ALERTS[Math.floor(Math.random() * NEW_ALERTS.length)],
        isNew: true
      };
    ...
    <div className="panel-pixel border-rust-900 bg-rust-950/40 py-10 px-8 relative overflow-hidden">
      ...
      <div className="space-y-8 relative">
        {events.map((event) => (
          <div 
            key={event.id} 
            className={`flex gap-6 items-start p-4 transition-all duration-700 ${event.isNew ? 'bg-rust-900/40 border-l-4 border-terracotta-400 animate-slide-in' : 'opacity-80 hover:opacity-100'}`}
          >
